...
import 'package:dtxproject/screens/survey_screens/exercise_survey/exercise_survey_page_7.dart';
import 'package:dtxproject/screens/survey_screens/survey_home_page.dart';

class ExerciseSurveyPage6 extends StatelessWidget {
  final surveyController = Get.find<SurveyController>();

  ExerciseSurveyPage6({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            // 문항
                            const Text(
                              '6. 평소 최소 10분 이상 계속 숨이 약간  차거나 심장이 약간 빠르게 뛰는 중강도의 스포츠, 운동 및 여가 활동을 하십니까? 혹은 직업적으로 이러한 일을 최소 10분 이상 포함하십니까?',
                              style: TextStyle(
                                  fontSize: 16, fontFamily: 'Paperlogy'),
                            ),
                            const SizedBox(height: 10),
                            const Text(
                              '중강도 신체활동 : 빠르게 걷기(일하는 중에), 가벼운 물건 나르기, 청소, 육아(목욕시키기, 아이 안아주기 등)빠르게 걷기, 가볍게 뛰기(조깅), 웨이트 트레이닝(근력 운동), 골프, 댄스스포츠, 필라테스 등',
                              style: TextStyle(
                                      ...
                                      return Obx(
                                        () {
                                          ...
                                          bool isSelected = surveyController
                                                  .exerciseQ6Option.value ==
                                              index;
                                          return GestureDetector(
                                            onTap: () => surveyController
                                                .exerciseQ6Option.value = index,
      ...
      bottomNavigationBar: Obx(() {
        bool isButtonEnabled = surveyController.exerciseQ6Option.value !=
          ...
          child: SizedBox(
            ...
            child: ElevatedButton(
              ...
              onPressed: isButtonEnabled
                  ? () {
                      if (surveyController.exerciseQ6Option.value == 0) {
                        // "전혀 하지 않았다" 선택 시, 다음 질문 생략 & 설문 홈으로 이동
                        surveyController.completeExerciseSurvey();
                        Get.to(() => const SurveyHomePage());
                      } else {
                        // 1일 이상 한 경우 → 다음 페이지로 이동
                        Get.to(() => ExerciseSurveyPage7());
                      }
                    }
                  : null,
              child: const Text(
